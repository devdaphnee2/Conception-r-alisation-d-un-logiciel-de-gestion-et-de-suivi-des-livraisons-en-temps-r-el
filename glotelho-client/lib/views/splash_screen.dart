  ...
  Future<void> _navigate() async {
    ...
    if (state.isLoggedIn) {
      ...
      if (state.biometricEnabled) {
        ...
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(builder: (_) => authenticated
            ? const HomeScreen()
            : const LoginScreen()),
        );
        return;
        ...
        child: Center(
          child: Column(
            ...
            children: [
              ...
              const Text('Glotelho', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w600, color: Colors.white)),
              const SizedBox(height: 4),
              const Text('EXPRESS', style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, letterSpacing: 4, color: gold)),
              const SizedBox(height: 32),
              const SizedBox(width: 24, height: 24,
                child: CircularProgressIndicator(strokeWidth: 2, valueColor: AlwaysStoppedAnimation<Color>(gold))),
            ],
          ),
        ),
      ),
