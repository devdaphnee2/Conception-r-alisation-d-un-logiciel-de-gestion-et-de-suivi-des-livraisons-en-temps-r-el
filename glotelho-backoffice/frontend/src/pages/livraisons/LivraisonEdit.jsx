                    ...
                    <div style={{ display: 'grid', gap: '20px' }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                            <div>
                                ...
                                <input
                                    type="text"
                                    name="zone_bloc"
                                    value={form.zone_bloc}
                                    onChange={handleChange}
                                    style={inputStyle}
                                    placeholder="Ex: Zone 3, Bloc B"
                                />
                            </div>
                            <div>
                        ...
                        <div style={{ display: 'flex', gap: '12px', marginTop: '8px' }}>
                            <button
                                type="submit"
                                disabled={saving}
                                style={{ background: 'linear-gradient(to right, #E8580A, #FF7A33)', color: 'white', padding: '12px 28px', borderRadius: '12px', border: 'none', fontSize: '14px', fontWeight: 600, cursor: saving ? 'not-allowed' : 'pointer', opacity: saving ? 0.7 : 1, boxShadow: '0 4px 12px rgba(232,88,10,0.25)', fontFamily: 'Poppins, sans-serif' }}
                            >
                                {saving ? 'Enregistrement...' : 'Enregistrer les modifications'}
                            </button>
                            <Link
                                to={`/livraisons/${id}`}
                                style={{ padding: '12px 24px', borderRadius: '12px', border: '1px solid #ECECF2', backgroundColor: '#FAFAFC', color: '#1C1C2E', fontSize: '14px', fontWeight: 500, textDecoration: 'none', display: 'inline-flex', alignItems: 'center' }}
                            >
                                Annuler
                            </Link>
                        </div>
