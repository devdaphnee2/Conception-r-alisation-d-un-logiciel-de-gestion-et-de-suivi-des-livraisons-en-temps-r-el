...
export default function LivraisonShow() {
    ...
    const btnPrimary = {
        ...
        borderRadius: '12px',
        border: 'none',
        background: 'linear-gradient(to right, #E8580A, #FF7A33)',
        color: 'white',
        fontSize: '14px',
        fontWeight: 600,
        cursor: 'pointer',
    ...
    return (
        <div style={{ maxWidth: '780px' }}>
            ...
            <div style={{ marginBottom: '24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div>
                    <Link to="/livraisons" style={{ color: '#8A8AA3', textDecoration: 'none', fontSize: '14px' }}>
                        Livraisons
                    </Link>
            ...
            <div style={cardStyle}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                    <div>
                        ...
                        <p style={valueStyle}>{livraison?.delivery_address}</p>
                    </div>
                    <div>
                        <p style={labelStyle}>Zone / Bloc</p>
                        <p style={valueStyle}>{livraison?.zone_bloc || '—'}</p>
                    </div>
                    <div>
                        <p style={labelStyle}>Montant a recouvrer</p>
                        <p style={valueStyle}>{livraison?.amount_to_collect ? `${livraison.amount_to_collect} FCFA` : '—'}</p>
                    </div>
                    <div>
                        <p style={labelStyle}>Date de livraison</p>
                        <p style={valueStyle}>{livraison?.delivery_date ? new Date(livraison.delivery_date).toLocaleString('fr-FR') : '—'}</p>
                    </div>
                    <div>
                        <p style={labelStyle}>Date de creation</p>
                        <p style={valueStyle}>{livraison?.creation_date ? new Date(livraison.creation_date).toLocaleString('fr-FR') : '—'}</p>
                    </div>
                    {livraison?.suspension_reason && (
                        <div>
                            <p style={labelStyle}>Raison de suspension</p>
                            <p style={{ ...valueStyle, color: '#DC2626' }}>{livraison.suspension_reason}</p>
                        </div>
                    )}
                </div>
            </div>
            ...
            <div style={{ ...cardStyle, marginBottom: 0 }}>
                ...
                <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                    ...
                    {['En_attente', 'Suspendu'].includes(livraison?.status) && (
                        <button
                            onClick={() => { setShowAssignForm(!showAssignForm); setShowSuspendForm(false); }}
                            style={btnPrimary}
                        >
                    ...
                    {['Assign_', 'En_cours'].includes(livraison?.status) && (
                        <button
                            onClick={() => { setShowSuspendForm(!showSuspendForm); setShowAssignForm(false); }}
                            style={btnSecondary}
                        >
                            Suspendre
                        </button>
                ...
                {showAssignForm && (
                    <div style={{ marginTop: '20px', padding: '20px', backgroundColor: '#FAFAFC', borderRadius: '16px', border: '1px solid #ECECF2' }}>
                        <p style={{ fontSize: '14px', fontWeight: 600, color: '#1C1C2E', marginBottom: '12px' }}>
                            Choisir un livreur
                        </p>
                        <select
                        ...
                        <div style={{ display: 'flex', gap: '10px', marginTop: '12px' }}>
                            <button
                                onClick={() => handleAction('assigner', { delivery_person_id: selectedLivreur })}
                                disabled={!selectedLivreur || actionLoading}
                                style={{ ...btnPrimary, opacity: !selectedLivreur ? 0.6 : 1 }}
                            >
                                {actionLoading ? 'En cours...' : 'Confirmer'}
                            </button>
                            <button onClick={() => setShowAssignForm(false)} style={btnSecondary}>
                                Annuler
                            </button>
                        </div>
                    </div>
                ...
                {showSuspendForm && (
                    <div style={{ marginTop: '20px', padding: '20px', backgroundColor: '#FAFAFC', borderRadius: '16px', border: '1px solid #ECECF2' }}>
                        ...
                        <textarea
                            ...
                            placeholder="Decrivez la raison de la suspension..."
                            rows={3}
                            style={{ ...inputStyle, resize: 'vertical' }}
                        />
                        <div style={{ display: 'flex', gap: '10px', marginTop: '12px' }}>
                            <button
                                onClick={() => handleAction('suspendre', { suspension_reason: suspensionReason })}
                                disabled={!suspensionReason || actionLoading}
                                style={{ ...btnDanger, border: 'none', backgroundColor: '#DC2626', color: 'white', opacity: !suspensionReason ? 0.6 : 1 }}
                            >
                                {actionLoading ? 'En cours...' : 'Confirmer la suspension'}
                            </button>
                            <button onClick={() => setShowSuspendForm(false)} style={btnSecondary}>
                                Annuler
                            </button>
                        </div>
                    </div>
