...
function KpiCard({ icon: Icon, label, value, iconColor, iconBg }) {
    return (
        <div style={{ backgroundColor: P.inverseS, borderRadius: '14px', padding: '18px', position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ width: '38px', height: '38px', borderRadius: '10px', backgroundColor: iconBg, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '14px' }}>
...
export default function Dashboard() {
    ...
    useEffect(() => {
        Promise.all([
            ...
            setStats({
                total: l.length,
                en_attente: l.filter(x => x.status === 'En_attente').length,
                assignees: l.filter(x => x.status === 'Assign_').length,
                livrees: l.filter(x => x.status === 'Livr_').length,
                litiges: litRes.data?.length || 0,
            });
    ...
    const livreurDot = {
        'Disponible':   '#4caf50',
        'En_livraison': P.primaryContainer,
        'Suspendu':     P.error,
        'Indisponible': P.outline,
    };
    ...
    return (
        <div style={{ fontFamily: 'Poppins, sans-serif' }}>
            ...
            {/* KPIs — sans pourcentages */}
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '12px', marginBottom: '20px' }}>
                <KpiCard icon={IconPackage} label="Total livraisons"  value={stats.total}      iconColor={P.primaryContainer} iconBg="rgba(201,149,46,0.15)" />
                ...
                <KpiCard icon={IconAlert}   label="Litiges"           value={stats.litiges}     iconColor="#ef9a9a"            iconBg="rgba(239,154,154,0.15)" />
            </div>
                    ...
                    <div style={{ backgroundColor: P.surface, borderRadius: '14px', border, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.05)' }}>
                        ...
                        <div>
                            {loading ? (
                                ...
                                <Link key={l.id} to={'/livreurs/' + l.id}
                                    style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 16px', textDecoration: 'none', borderBottom: '1px solid ' + P.surfaceContainerLow, transition: 'background 0.1s' }}
                                    onMouseEnter={e => e.currentTarget.style.backgroundColor = P.surfaceContainerLow}
                                    onMouseLeave={e => e.currentTarget.style.backgroundColor = 'transparent'}
                                >
