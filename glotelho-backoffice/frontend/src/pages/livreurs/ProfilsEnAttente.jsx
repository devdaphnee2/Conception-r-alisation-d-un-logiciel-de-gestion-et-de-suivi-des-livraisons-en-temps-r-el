...
export default function ProfilsEnAttente() {
    ...
    async function handleApprouver(id, nom) {
        ...
        try { const res = await api.post('/profils/' + id + '/approuver'); setSuccessMsg(res.data.message); charger(); }
    ...
    async function handleRejeter(id, nom) {
        ...
        if (!motif) return;
        setActionLoading(true);
        try { const res = await api.post('/profils/' + id + '/rejeter', { motif }); setSuccessMsg(res.data.message); charger(); }
        catch (err) { setError(err.response?.data?.message || 'Erreur.'); }
        finally { setActionLoading(false); }
    }
    ...
    return (
        <div style={{ fontFamily: 'Poppins, sans-serif' }}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '10px', marginBottom: '18px' }}>
                ...
                <KpiCard icon={IconAlert} label="Caution en attente"      value={stats.total - stats.cautionOk} iconColor="#ef9a9a" iconBg="rgba(239,154,154,0.15)" />
            </div>
            ...
            <div style={{ backgroundColor: P.surface, borderRadius: '14px', border: '1px solid ' + P.outlineVariant, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.05)' }}>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <thead><tr>
                        {['Candidat', 'Contact', 'Zone / Vehicule', 'Caution', 'Documents', 'Actions'].map(h => <th key={h} style={thStyle}>{h}</th>)}
                    </tr></thead>
                    <tbody>
                        {loading ? (
                            ...
                            return (
                                <tr key={l.id} onMouseEnter={e => e.currentTarget.style.backgroundColor = P.surfaceContainerLow} onMouseLeave={e => e.currentTarget.style.backgroundColor = 'transparent'} style={{ transition: 'background 0.1s' }}>
                                    ...
                                    <td style={tdStyle}>
                                        <div>
                                            <p style={{ margin: 0, fontSize: '13px', fontWeight: 700, color: cautionOk ? '#1b5e20' : P.error }}>
                                    ...
                                    <td style={tdStyle}>
                                        {dossierOk ? (
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                                                <IconCheck style={{ width: '14px', height: '14px', color: '#1b5e20' }} />
                                                <span style={{ fontSize: '11px', color: '#1b5e20', fontWeight: 600 }}>Complet</span>
                                            </div>
                                        ) : (
                                            <div>
                                                <div style={{ display: 'flex', alignItems: 'center', gap: '5px', marginBottom: '3px' }}>
                                                    <IconX style={{ width: '13px', height: '13px', color: P.error }} />
                                                    <span style={{ fontSize: '11px', color: P.error, fontWeight: 600 }}>{docsManquants.length} manquant{docsManquants.length > 1 ? 's' : ''}</span>
                                                </div>
                                                {docsManquants.map((d, i) => <p key={i} style={{ margin: 0, fontSize: '10px', color: d === 'Photo profil' ? P.error : P.outline, fontWeight: d === 'Photo profil' ? 700 : 400 }}>• {d}</p>)}
                                            </div>
                                        )}
                                    </td>
                                    <td style={tdStyle}>
                                        <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                                            ...
                                            <button onClick={() => handleApprouver(l.id, nom)} disabled={actionLoading}
                                            ...
                                            <button onClick={() => handleRejeter(l.id, nom)} disabled={actionLoading}
                ...
                <div style={{ padding: '11px 16px', borderTop: '1px solid ' + P.surfaceContainerLow }}>
                    <p style={{ margin: 0, fontSize: '11px', color: P.outline }}>{filtered.length} profil{filtered.length > 1 ? 's' : ''} affiche{filtered.length > 1 ? 's' : ''}</p>
                </div>
            </div>
        </div>
    );
